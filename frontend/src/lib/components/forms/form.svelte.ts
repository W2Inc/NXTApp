 ...
 * @returns
 */
export function useForm<T extends Record<string, unknown>>(formData: SuperValidated<T>, options?: FormOptions) {
	...
	return superForm(formData, {
		...
		async onSubmit({ cancel }) {
			if (formOptions.confirm !== undefined) {
				const c = await useDialog.confirm(formOptions.confirm);
				if (!c) {
					cancel();
					return;
				}
