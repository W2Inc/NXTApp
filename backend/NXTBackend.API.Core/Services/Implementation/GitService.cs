...
public sealed class GitService2 : BaseService<Git>, IGitService
{
        ...
        IHttpContextAccessor httpContext) : base(ctx)
    {
        ...
        _client = clientFactory.CreateClient("NXTGit");
    }
    ...
    public async Task<IEnumerable<GitVfsNodeDO>> GetFiles(Guid id, string path, string branch)
    {
        ...
        {
            // Note(W2): Gitea responds with array when targeting a directory but a single object
            // when targeting a file.
            if (!string.IsNullOrEmpty(Path.GetExtension(path)))
                return [await response.Content.ReadFromJsonAsync<GitVfsNodeDO>() ?? throw new ServiceException()];
            else
                return await response.Content.ReadFromJsonAsync<GitVfsNodeDO[]>() ?? throw new ServiceException();
        }
